    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += '<i class="easyUploadIcon noselect head_check" data-checked="no" style="opacity: 0;">&#xe693;</i>';
          // $html += option.showNote ? '<span class="easy_upload_note">' + option.note + '</span>' : '';
          $html += '</div>';
          $html += '<ul class="easy_upload_queue"></ul>';
          $html += option.showNote ? '<span class="easy_upload_note">' + option.note + '</span>' : '';
          $html += '</div>';
          $('body').append( '<span class="easy_upload_head_btn1 noselect">上传</span>')
          _ele.html($html);
          this.bindHead();
        },
        bindHead: function () {
          ...
          $('.fileInput').off('change').change(function () {
            ...
            $(this).parent().find('.head_check').html('&#xe693;').attr('data-checked', 'no');
              var opt = {type: 'all', target: $('.easy_upload-head i')};
        ...
        _renderFile: function (fileArr, target) {
          $('.easy_upload-head .easy_upload_select').hide();
