...
class Redfilm extends Model {
    ...
    public function get_list($length,$len){
        ...
        $details = $this ->where($map) ->order('create_time desc') ->limit($length,$len) ->field("'redfilm' as genre, id, title, create_time, front_cover, '' as publisher, status, recommend, views, comments, likes") ->select();
        return $details;
    }
}